...
open class MockManager {
    ...
    fileprivate var stubCalls: [StubCall] = []
    fileprivate var unverifiedStubCallsIndexes: [Int] = []
    
    public init() {
        
    }

    open func getter<T>(_ property: String, original: ((Void) -> T)? = nil) -> T {
        return call(getterName(property), parameters: Void(), original: original)
    }
    
    open func setter<T>(_ property: String, value: T, original: ((T) -> Void)? = nil) {
        return call(setterName(property), parameters: value, original: original)
    }
    
    open func call<IN, OUT>(_ method: String, parameters: IN, original: ((IN) -> OUT)? = nil) -> OUT {
        return try! callThrows(method, parameters: parameters, original: original)
    }
    
    open func callThrows<IN, OUT>(_ method: String, parameters: IN, original: ((IN) throws -> OUT)? = nil) throws -> OUT {
    ...
    open func createStub<IN, OUT>(_ method: String, parameterMatchers: [ParameterMatcher<IN>]) -> ConcreteStub<IN, OUT> {
        let stub = ConcreteStub<IN, OUT>(method: method, parameterMatchers: parameterMatchers)
        stubs.insert(stub, at: 0)
        return stub
    }
    
    open func verify<IN, OUT>(_ method: String, callMatcher: CallMatcher, parameterMatchers: [ParameterMatcher<IN>], sourceLocation: SourceLocation) -> __DoNotUse<OUT> {
        var calls: [StubCall] = []
    ...
    fileprivate func failAndCrash(_ message: String, file: StaticString = #file, line: UInt = #line) -> Never  {
